package com.backend.avatar.constant;

public class OpenApiConstant {
    public static final String NAME_API                     = "API Avatares";
    public static final String VERSION_API                  = "v1.0.0";
    public static final String DESCRIPTION_API              = "Este es el microservicio avatares";

    public static final String TEXT_CONTROLLER              = "Operaciones relacionadas con ";

    public static final String URL_SERVER_LOCAL             = "http://localhost:8080";
    public static final String DESCRIPTION_SERVER_LOCAL     = "Servidor local";

    public static final String AVATAR_CONTROLLER            = "avatar-controller";
    public static final String IMAGE_CONTROLLER             = "image-controller";
}
